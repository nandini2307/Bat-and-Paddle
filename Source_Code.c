...
const float ball_maxu = 9.5; // up const
float ball_maxd = -9.5; // down const
float ball_maxr = 15; // right
const float ball_maxl = -15; // left
bool isReachedXMax = false; bool
isReachedYMax = false; bool
isGameEnd = false;
bool isGameOver = false;
/* Start the scene at 30 unit backward from center */ const
int zoom = -30;
/* Levels and points boundary */
const int level_a = 3; const
int level_b = 6;

const int level_c = 9;
/* Player configs */ static
int player_score = 0; static
int player_lives = 3;
static int level = 1;
/* Text buffer */
char finaltext[100];
...
glColor3f(0.8, 0, 0.1); glutSolidSphere(ball.r,
20, 20); glPopMatrix();
// Bottom bar
glPushMatrix();
...
glColor3f(0, 1, 0);
glVertex2f(bar.w, -bar.h);
glColor3f(0, 0, 1); glVertex2f(-
bar.w, -bar.h); glColor3f(1, 0, 1);
glVertex2f(-bar.w, bar.h);
glEnd();
glPopMatrix();
// Top bar
...
glColor3f(0, 0, 1);
glVertex2f(-top_bar.w, -top_bar.h);
glColor3f(1, 0, 1); glVertex2f(-top_bar.w,
top_bar.h);
glEnd();
glPopMatrix();
glutSwapBuffers();
...
/* Reset bar */
bar.y = -11;
/* If player has no life left end
the game and show black banner */
if(player_lives==0)
{
glClearColor(0, 0, 0, 1);
isGameEnd = true; isGameOver
= true;
...
{
glClearColor(0, 0.8, 0, 1); strcpy(finaltext,
&quot;Congratulation! Top score recorded&quot;); isGameEnd =
true;
}
}
}
...
else ball.x += ball.xstep *
ball.velocity; if(isReachedYMax)
ball.y -= ball.ystep * ball.velocity;
else ball.y += ball.ystep *
ball.velocity;
